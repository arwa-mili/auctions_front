...
//Types
import { AutoCompleteProps, ColorInputProps } from "./autocomplete.types";
...
function AutoComplete<T>(props: AutoCompleteProps<T>): JSX.Element {
  ...
  const onChange = (
    ...
    clearErrors(name);
    if (nameSearch) {
      setValue(nameSearch, value ? getOptionLabel?.(value) : "");
    }
    if (props.onChange) {
  ...
  const renderEndAdornment = (params: AutocompleteRenderInputParams) => {
    if (params.InputProps.endAdornment && !disabled) {
